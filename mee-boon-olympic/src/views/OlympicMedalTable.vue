          ...
          <tbody>
            <template v-for="(country, index) in countryStore.paginatedCountries" :key="country.id">
              <tr class="border-b border-gray-200 hover:bg-gray-50 transition duration-150">
                ...
                <td class="py-3 px-4 text-center font-semibold text-yellow-500">
                  {{ country.gold }}
                </td>
                <td class="py-3 px-4 text-center font-semibold text-gray-500">
                  {{ country.silver }}
                </td>
                ...
                <td class="py-3 px-4 text-center">
                  <RouterLink
                    ...
                    class="inline-block py-2 px-4 bg-blue-500 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75 transition duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-105"
                  >
                    View more details
                  </RouterLink>
                </td>
      ...
      <div>
        <template v-if="countryStore.currentPage > 1">
          <RouterLink
            ...
            class="px-3 py-2 bg-gray-300 rounded cursor-pointer"
          >
            Previous
          </RouterLink>
        </template>
        <span
          v-else
          class="px-3 py-2 bg-gray-300 rounded cursor-not-allowed opacity-50"
        >
          Previous
        </span>
      </div>
